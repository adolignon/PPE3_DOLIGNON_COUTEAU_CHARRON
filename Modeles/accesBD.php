...
//            $this->boolConnexion = true;
	...
	public function updateMdp($unMdp,$unLogin){
		$requete = $this->conn->prepare("UPDATE client SET pwd = '".$unMdp."' WHERE login = '".$unLogin."'");
		if(!$requete->execute())
		{
    ...
    public function updateNom($unNom,$unLogin){
        $requete = $this->conn->prepare("UPDATE client SET nomClient = '".$unNom."' WHERE login = '".$unLogin."'");
        if(!$requete->execute())
        {
            die("Erreur dans updateNom : ".$requete->errorCode());
        }
    }

    public function updateEmail($unEmail,$unLogin){
        $requete = $this->conn->prepare("UPDATE client SET emailClient = '".$unEmail."' WHERE login = '".$unLogin."'");
        if(!$requete->execute())
        {
            die("Erreur dans updateEmail : ".$requete->errorCode());
        }
    }
	//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
